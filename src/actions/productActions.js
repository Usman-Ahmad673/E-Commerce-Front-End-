...
export const getProduct = (keyword = "" , currentPage = 1 , price = [0 , 320000] , category , ratings = 0) => async(dispatch) =>{
    try{
        ...
        // }
        // else{
        //     link = `/api/v1/products/?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings=${ratings}`
        // }
...
export const updateProduct = (id , updatedProductData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`/api/v1/admin/product/${id}`, updatedProductData, config);
        console.log(data);
